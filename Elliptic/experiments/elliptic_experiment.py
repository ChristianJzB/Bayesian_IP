...
from elliptic_files.utilities import generate_noisy_obs,deepgala_data_fit,run_da_mcmc_chain,run_mcmc_chain
...
def run_experiment(config_experiment,device):
    ...
    if config_experiment.da_mcmc_nn:
        ...
        for i in range(config_experiment.repeat):
            ...
            np.save('./Elliptic/results/NN'+ model_specific+ f'_var{config_experiment.noise_level}_{i}.npy', nn_samples)
            np.save('./Elliptic/results/mcmc_da_nn' +model_specific+ f'_{config_experiment.noise_level}_{i}.npy', acceptance_res)
            np.save("./Elliptic/results/mcmc_da_nn_proposal_thetas" + model_specific + f"_var{config_experiment.noise_level}_{i}.npy", proposal_thetas)
            np.save("./Elliptic/results/mcmc_da_nn_lh_nn" + model_specific + f"_var{config_experiment.noise_level}_{i}.npy", lh_val_nn)
            np.save("./Elliptic/results/mcmc_da_nn_lh_solver" + model_specific + f"_var{config_experiment.noise_level}_{i}.npy", lh_val_solver)

    # Step 8: Delayed Acceptance for Dgala
    if config_experiment.da_mcmc_dgala:
        print(f"Starting MCMC-DA with DGALA_s{config_experiment.nn_samples} and FEM for {config_experiment.repeat}times")
        llp = torch.load(path_dgala_model, map_location=device)
        llp.model.set_last_layer("output_layer")  # Re-register hooks
        llp._device = device
        ...
        for i in range(config_experiment.repeat):
            ...
            times["dgala_da"] = tda
            np.save('./Elliptic/results/dgala'+model_specific+ f'_var{config_experiment.noise_level}_{i}.npy', nn_samples)
            np.save('./Elliptic/results/mcmc_da_dgala' +model_specific+f'_{config_experiment.noise_level}_{i}.npy', acceptance_res)
            np.save("./Elliptic/results/mcmc_da_dgala_proposal_thetas" + model_specific + f"_var{config_experiment.noise_level}_{i}.npy", proposal_thetas)
            np.save("./Elliptic/results/mcmc_da_dgala_lh_nn" + model_specific + f"_var{config_experiment.noise_level}_{i}.npy", lh_val_nn)
            np.save("./Elliptic/results/mcmc_da_dgala_lh_solver" + model_specific + f"_var{config_experiment.noise_level}_{i}.npy", lh_val_solver)

    with open(f'./Elliptic/results/times/timing_nn'+ model_specific+ '.json', 'w') as f:
