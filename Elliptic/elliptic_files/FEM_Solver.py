...
class Parametric_K:
    """Class defining the k(x) function for the PDE"""
    ...
    def eval(self, x):
        ...
        phi = self.A[:,None] * (np.sin(self.roots[:,None] * X[None, :]) + (self.roots[:,None] / 4) * np.cos(self.roots[:,None] * X[None, :]))

        result= np.sum(phi*self.an[:,None]*self.theta[:,None], axis=0)
