...
from Base.utilities import UniformSampler
...
def train_elliptic(config, device):
    ...
    #batch_size = int(config.nn_samples / config.n_batches)
    #batch_size = 10 if config.nn_samples < 1000 else 100
    ...
    for epoch in range(config.epochs):
        ...
        for data_int, left_bc, right_bc in dataloader:
            ...
            total_loss,losses = dg_elliptic.total_loss(data_int, left_bc, right_bc, loss_fn,update_weights=update_weights)
...
def get_dom(d, device='cpu'):
    # Start with the first row [0, 1]
    dom_list = [[0, 1]]
    # Then append [-1, 1] (d - 1) times
    dom_list += [[-1, 1]] * (d)
    # Convert to tensor and move to device
    return torch.tensor(dom_list, dtype=torch.float).to(device)

def data_preparing(batch):
    rows, cols = batch.shape
    param_tensor = batch[:, 1:]

    # If param_tensor is 1D, make it 2D
    if param_tensor.ndim == 1:
        param_tensor = param_tensor.unsqueeze(-1)

    left_bc = torch.cat([torch.zeros((rows,1)), param_tensor], dim=1).float()
    right_bc = torch.cat([torch.ones((rows,1)), param_tensor], dim=1).float()

    return batch, left_bc, right_bc

def train_dg_elliptic(config, device):
    """Train the PINN using the Adam optimizer with early stopping, computing test loss after each epoch."""
    wandb_config = config.wandb
    wandb.init(project=wandb_config.project, name=wandb_config.name)
    ...
    dom =get_dom(config.KL_expansion, device=device)
    samples_interior = iter(UniformSampler(dom,config.nn_samples,device = device))
    ...
    for epoch in range(config.epochs):
        ...
        data_int, left_bc, right_bc = data_preparing(batch)
