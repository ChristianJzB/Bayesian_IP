...
from .elliptic_mcmc import EllipticMCMC, EllipticMCMCDA
...
def deepgala_data_fit(samples,nparameters,device,seed = 65647437836358831880808032086803839626):
    ...
    # dataset = dGDataset(size = samples, param=param_train)
    ...
    # dgala_data = {"data_fit": {dataloader}, 
    #             "class_method": {"pde": ["elliptic_pde"], "left_bc":["u"],"right_bc":["u"]},
    #             "outputs": {"pde": ["elliptic"], "left_bc": ["ubcl"],"right_bc":["ubcr"]}, 
    #             "other":{"time_dependent":False,}}
    return dgala_data
...
def generate_noisy_obs(obs, nparam=2,vert=1000, mean=0, std=np.sqrt(1e-4)):
    ...
    """
    theta_thruth = np.array([0.098, 0.430, 0.206, 0.090, -0.153,
...
def pigp_training_data_generation(theta_obs,spatial_points_obs,kl_expansion, device):
    ...
    obs_points = np.linspace(0.2,0.8,spatial_points_obs).reshape(-1,1)
