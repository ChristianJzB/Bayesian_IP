...
from nv_files.Pseudo_Spectral_Solver import solve_poisson_fft,compute_velocity
...
def train_vorticity_dg(config,device):
    ...
    # Domain and sampler setup
    dom = torch.tensor([[0, 2 * torch.pi], [0, 2 * torch.pi],[0,config.time_domain]]).to(device)
    samples_interior = iter(UniformSampler(dom, batch_size_interior,device = device,rng_seed=1234))
...
class dGDataset(Dataset):
    def __init__(self, sorted_batch,initial_points_,initial_condition):
        self.size = sorted_batch.shape[0]

        self.col_point = sorted_batch
        self.col_init =  initial_points_
        self.value_init = initial_condition 

    def __len__(self):
        return self.size

    def __getitem__(self, index):
        return self.col_point[index], self.col_init[index], self.value_init[index]
    

def train_vorticity_epoch(config,device):
    """Train the PINN model with a timer for loss computation."""
    # Initialize W&B and environment
    wandb_config = config.wandb
    wandb.init(project=wandb_config.project, name=wandb_config.name)
    ...
    dataset = dGDataset(sorted_batch,initial_points_,initial_condition)
    ...
    for epoch in range(config.iterations):
        ...
        for batch, initial_points_, initial_condition in dataloader:
            _, indices = batch[:, 2].sort()  # Sort based on the time column
