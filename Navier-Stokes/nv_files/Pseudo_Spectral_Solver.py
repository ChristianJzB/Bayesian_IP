...
def solve_poisson_fft(omega, dx, dy):
    ...
    # Compute wavenumbers
    kx = torch.fft.fftfreq(nx, dx) * 2 * torch.pi  # Wavenumbers in x
    ky = torch.fft.fftfreq(ny, dy) * 2 * torch.pi  # Wavenumbers in y
    kx, ky = torch.meshgrid(kx, ky, indexing="ij")
    k2 = kx**2 + ky**2
    k2[0, 0] = 1.0  # Avoid division by zero at (kx, ky) = (0, 0)

    # FFT of vorticity (batched along the last dimension)
    omega_hat = torch.fft.fft2(omega, dim=(0, 1))
    ...
    psi = torch.fft.ifft2(psi_hat, dim=(0, 1)).real
    return psi
...
def compute_velocity(psi, dx, dy):
    ...
    psi_hat = torch.fft.fft2(psi, dim=(0, 1))
    ...
    u = torch.fft.ifft2(u_hat, dim=(0, 1)).real
    v = torch.fft.ifft2(v_hat, dim=(0, 1)).real
    return u, v