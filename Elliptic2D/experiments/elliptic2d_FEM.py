...
def run_experiment(config_experiment,device):
    ...
    for i in range(config_experiment.repeat):
        ...
        np.save(f'./Elliptic2D/results/e2dFEM_kl{config_experiment.KL_expansion}_var{config_experiment.noise_level}_{i}.npy', fem_samples[0])
        np.save(f'./Elliptic2D/results/e2dFEM_eval_kl{config_experiment.KL_expansion}_var{config_experiment.noise_level}_{i}.npy', fem_samples[-1])
