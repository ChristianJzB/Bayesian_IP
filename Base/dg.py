...
from .deep_models import DNN,WRFNN, MDNN

# Model options for the DGM
def _uplad_model(config):
    # Simple Feedforward NN
    if config.nn_model == "NN":
        model = DNN(**config.model)
    #NN with random weight factorization 
    elif config.nn_model == "WRF":
        model = WRFNN(**config.model)
    # Modified NN the one that used in the work
...
class deepGalerkin(torch.nn.Module):
    ...
    # This docarotr is used for the lla class to fir the laplace approximation
    def laplace_approx():
        def decorator(func):
    ...
    # This used for time dependet PDES specially to create the M slots for the time axis
    ...
    # Gets all the losses and updates the weights of each one
    ...
    # Updates the weights acording to the weightsed gradient scheme
