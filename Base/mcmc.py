...
class MetropolisHastings(torch.nn.Module):
    ...
    def __init__(self, observation_locations, observations_values, nparameters=2, 
        ...
        self.burnin = int(self.nsamples * 0.25) if burnin is None else burnin
...
class MCMCDA(torch.nn.Module):
    """
    Delayed Acceptance, first runs the MCMC then DAMCMC
    """
    def __init__(self, observation_locations, observations_values,mcmc_class, nparameters=2, 
        ...
        if proposal_type == "pCN":
            self.log_prior_func = None  # Or set to a dummy function that returns 0
            self.posterior_distribution_outer = self.log_likelihood_outer
            self.posterior_distribution_inner = self.log_likelihood_inner

        elif proposal_type in prior_methods:
            self.log_prior_func = prior_methods[proposal_type]
            self.posterior_distribution_outer= (lambda theta: self.log_prior_func(theta) + self.log_likelihood_outer(theta))
            self.posterior_distribution_inner= (lambda theta: self.log_prior_func(theta) + self.log_likelihood_inner(theta))
        else:
            raise ValueError(f"Prior for proposal type '{proposal_type}' is not supported.")        
        
        self.mcmc_chain = mcmc_class
    ...
    def run_chain(self, verbose=True):
        ...
        inner_timer = Timer(use_gpu=True)
        inner_timer.start()
        ...
        while inner_mh < self.iter_da:
            ...
            if torch.rand(1, device=self.device) < a:
                ...
                # Save results of the values of the surrogates and solver and proposed thetas
                ...
                if torch.rand(1, device=self.device) < a:
                    ...
                    log_posterior_outer = log_posterior_proposal_outer
                    log_posterior_inner = log_posterior_proposal_inner
        ...
        time_inner = inner_timer.stop()
        # End progress bar
        if verbose:
            pbar.close()
            print(f"Times inner step {inner_mh:.4f}, Acceptance Rate: {inner_accepted / inner_mh:.4f}")

        return samples_outer,acceptance_list.cpu(),theta_inner.cpu(),likelihoods_val_nn.cpu(),likelihoods_val_solver.cpu(),time_outer,time_inner
